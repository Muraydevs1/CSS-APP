...
const MobileMenu = ({ setMobileMenuOpen }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 md:hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-xs w-full bg-green-800 text-white shadow-xl flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-green-700">
          <h2 className="text-xl font-bold">Menu</h2>
          <button onClick={closeMenu} className="p-2 rounded-md hover:bg-green-700">
            <FaTimes className="h-6 w-6" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto py-4">
          <nav className="flex flex-col space-y-1 px-2">
            <Link to="/" className="flex items-center px-4 py-3 rounded-md hover:bg-green-700" onClick={closeMenu}>
              <FaHome className="mr-3 h-5 w-5" />
              Home
            </Link>
            <Link
              to="/courses"
              className="flex items-center px-4 py-3 rounded-md hover:bg-green-700"
              onClick={closeMenu}
            >
